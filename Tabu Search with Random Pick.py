...
class TSP:
    ...
    def tabu_search_random(self):
        ...
        while run <= 10: #number of repetitions of the whole tabu_search algorithm
            tabulistlength = random.randint(1,20) #tabu tenure
            while times <= maxiter:
                dist_old = math.inf
                dist_cur_sol = 0
                for k in range(0, self.NumCust + 1):
                    dist_cur_sol += self.DistMatrix[seq_cur_sol[k]][seq_cur_sol[k + 1]]

                while dist_old > dist_cur_sol:
                    seq_old = seq_cur_sol.copy()
                    i = random.randint(1, self.NumCust) # Diversification element 1 in the solution
                    j = random.randint(1, self.NumCust) # Diversification element 2 in the solution
